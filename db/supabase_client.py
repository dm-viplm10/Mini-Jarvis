...
class SupabaseMemory:
    ...
    async def store_message(
        ...
        if data:
            message_obj["data"] = data  # type: ignore[assignment]
    ...
    async def store_plan(
        self, session_id: str, query: str, workflow: Dict, results: Dict
    ):
        ...
        try:
            ...
            # Store in Supabase
            self.client.table("plans").insert(
                {
                    "query": query,
    ...
    async def find_similar_plans(
        self, query: str, similarity_threshold: float = 0.8, limit: int = 5
    ):
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Error finding similar plans: {str(e)}"
            )
